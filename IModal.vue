<template>
  ...
  <br>
  formModalTemplate 的值为: {{formModalTemplate}}
</template>
...
// import { modal, modalFormTemplate } from "@/components";
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      modal({
        ...
        data: { test: "第一个input的数据", test1: "第二个input的数据", test2: '通过v-modal绑定input中的数据' },
        footer: [
          {
            onClick: (data, close) => {
              console.log(data);
